 ...
 */
class Digital8_Enrollments2_Model_Resource_Location extends Mage_Core_Model_Resource_Db_Abstract{
	/**
	 * constructor
	 * @access public
	 * @return void
	...
	public function _construct(){
		$this->_init('enrollments2/location', 'entity_id');
	}
	...
	public function lookupStoreIds($locationId){
		...
		$select  = $adapter->select()
			->from($this->getTable('enrollments2/location_store'), 'store_id')
	 ...
	 * @return Digital8_Enrollments2_Model_Resource_Location
	 * @author Ultimate Module Creator
	 */
	 ...
	 * @param string $field
	 * @param mixed $value
	 * @param Digital8_Enrollments2_Model_Location $object
	...
	protected function _getLoadSelect($field, $value, $object){
		...
		if ($object->getStoreId()) {
			...
			$select->join(
				array('enrollments2_location_store' => $this->getTable('enrollments2/location_store')),
				$this->getMainTable() . '.entity_id = enrollments2_location_store.location_id',
				array()
			)
			->where('enrollments2_location_store.store_id IN (?)', $storeIds)
			->order('enrollments2_location_store.store_id DESC')
			->limit(1);
		}
		return $select;
	}
	...
	protected function _afterSave(Mage_Core_Model_Abstract $object){
		...
		$table  = $this->getTable('enrollments2/location_store');
	...
	protected function _initCheckUrlKeySelect($urlKey, $store){
		$select = $this->_getReadAdapter()->select()
			...
			->join(
				array('es' => $this->getTable('enrollments2/location_store')),
