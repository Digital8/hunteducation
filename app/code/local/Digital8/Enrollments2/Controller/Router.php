 ...
 */
class Digital8_Enrollments2_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract{
	/**
	 * init routes
	 * @access public
	 * @param Varien_Event_Observer $observer
	 * @return Digital8_Enrollments2_Controller_Router
	 * @author Ultimate Module Creator
	 */
	public function initControllerRouters($observer){
		$front = $observer->getEvent()->getFront();
		$front->addRouter('enrollments2', $this);
		return $this;
	}
	/**
	...
	public function match(Zend_Controller_Request_Http $request){
		...
		$check['intake'] = new Varien_Object(array(
			'model' =>'enrollments2/intake',
			'controller' => 'intake',
			'action' => 'view',
			'param'	=> 'id',
		));
		$check['location'] = new Varien_Object(array(
			'model' =>'enrollments2/location',
			'controller' => 'location',
			'action' => 'view',
			'param'	=> 'id',
		));
		$check['enrollment'] = new Varien_Object(array(
			'model' =>'enrollments2/enrollment',
		...
		foreach ($check as $key=>$settings){
			...
			if ($id){
				$request->setModuleName('enrollments2')
