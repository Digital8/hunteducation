 ...
 */
class Digital8_Timetable_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract{
	/**
	 * init routes
	 * @access public
	 * @param Varien_Event_Observer $observer
	 * @return Digital8_Timetable_Controller_Router
	 * @author Ultimate Module Creator
	 */
	public function initControllerRouters($observer){
		$front = $observer->getEvent()->getFront();
		$front->addRouter('timetable', $this);
		return $this;
	}
	/**
	...
	public function match(Zend_Controller_Request_Http $request){
		...
		$check['enrolment'] = new Varien_Object(array(
			'model' =>'timetable/enrolment',
		...
		foreach ($check as $key=>$settings){
			...
			if ($id){
				$request->setModuleName('timetable')
