 ...
 */
class Digital8_Enrollments_Block_Adminhtml_Enrollment_Grid extends Mage_Adminhtml_Block_Widget_Grid{
	/**
	 * constructor
	 * @access public
	 * @return void
	 ...
	 * prepare collection
	 * @access protected
	 * @return Digital8_Enrollments_Block_Adminhtml_Enrollment_Grid
	 * @author Ultimate Module Creator
	 */
	...
	protected function _prepareColumns(){
		...
		$intakes = Mage::getResourceModel('enrollments/intake_collection')->toOptionHash();
		$this->addColumn('intake_id', array(
			'header'	=> Mage::helper('enrollments')->__('Intake'),
			'index'		=> 'intake_id',
			'type'		=> 'options',
			'options'	=> $intakes
		));
		$this->addColumn('uuid', array(
			'header'=> Mage::helper('enrollments')->__('uuid'),
			'index' => 'uuid',
			'type'	 	=> 'text',

		));
		$this->addColumn('approved', array(
			'header'=> Mage::helper('enrollments')->__('approved'),
			'index' => 'approved',
			'type'		=> 'options',
			'options'	=> array(
				'1' => Mage::helper('enrollments')->__('Yes'),
				'0' => Mage::helper('enrollments')->__('No'),
			)

		));
		$this->addColumn('email', array(
			'header'=> Mage::helper('enrollments')->__('email'),
			'index' => 'email',
			'type'	 	=> 'text',
	...
	protected function _prepareMassaction(){
		...
		$this->getMassactionBlock()->addItem('approved', array(
			'label'=> Mage::helper('enrollments')->__('Change approved'),
		...
		$values = Mage::getResourceModel('enrollments/intake_collection')->toOptionHash();
		$values = array_reverse($values, true); 
		$values[''] = ''; 
		$values = array_reverse($values, true);
		$this->getMassactionBlock()->addItem('intake_id', array(
			'label'=> Mage::helper('enrollments')->__('Change Intake'),
	 ...
	 * get the row url
	 * @access public
	 * @param Digital8_Enrollments_Model_Enrollment
	 * @return string
	 * @author Ultimate Module Creator
	 */
	 ...
	 * filter store column
	 * @access protected
	 * @param Digital8_Enrollments_Model_Resource_Enrollment_Collection $collection
